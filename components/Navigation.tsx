...
export default function Navigation({ locale }: { locale: Locale }) {
  const base = `/${locale}`
  return (
    <header className="bg-gray-950 sticky top-0 z-50 border-b border-gray-700">
      <nav className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6 text-sm font-medium text-gray-400">
          ...
          <Link
            ...
            target="_blank"
            rel="noopener"
            className="px-4 py-2 bg-gold-500 text-gray-950 font-semibold rounded hover:bg-gold-400 no-underline transition-colors text-sm"
          >
            Get a Quote
          </Link>
        </div>
