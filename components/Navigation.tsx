...
export default function Navigation({ locale }: { locale: Locale }) {
  ...
  const navLinks = [
    { label: isZh ? '知識庫' : 'KNOWLEDGE', href: `${base}/wiki` },
    { label: isZh ? '指南' : 'GUIDES',      href: `${base}/guide` },
    { label: isZh ? '工具' : 'TOOLS',       href: `${base}/tools` },
    { label: isZh ? '關於' : 'ABOUT',       href: `${base}/about` },
  ]
    ...
    <>
      ...
      >
        <nav className="max-w-[1200px] mx-auto px-4 lg:px-6 h-[60px] flex items-center justify-between gap-4">
          ...
          {/* Right side: locale switcher + search + CTA */}
